get_cache_infos <- function(){
  ...
  tibble(file=file_names) %>%
    ...
    # only extract the location_id e.g. suralaya from dispersion.suralaya.gdas1.310.120.20230616.RDS
    # using a match
            location_id = str_match(file, 'dispersion\\.(.*)\\.gdas1')[,2]) %>%
    select(date, location_id) %>%
    # convert to date
    mutate(date = as.Date(date, format='%Y%m%d'))
}

get_latest_continous_date <- function(plants, date_from="2023-01-01"){
  ...
  get_cache_infos() %>%
    ...
    summarise(latest_date_no_gap = if(any(date_diff > 1)){lag(date, default = date_from)[1]}else{min(date)}) %>%
